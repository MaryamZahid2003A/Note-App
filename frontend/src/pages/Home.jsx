...
function Home() {
  ...
  const fetchNotes = async () =>{
    ...
    try{
      ...
      const {data} = await axios.get("http://18.209.58.197:5000/api/note",{
        headers: {
  ...
  const addNote = async(title,description)=>{
    ...
    try{
                 const response = await axios.post("http://18.209.58.197:5000/api/note/add", {
                    title,
                    description,
                 }, {
                    headers: {
  ...
  const editNote = async (id, title, description)=>{
    try{
      const response = await axios.put(`http://18.209.58.197:5000/api/note/${id}`, {
         title,
         description,
      }, {
         headers: {
  ...
  const deleteNote = async (id)=>{
    try{
      const response = await axios.delete(`http://18.209.58.197:5000/api/note/${id}`,{
         headers: {
             Authorization: `Bearer ${localStorage.getItem("token")}`,
